...
class GetaccountMutation(graphene.Mutation):
    ...
    def mutate(self, info, company=1, **kwargs):
        ...
        try:
            ...
            account.company_id = company_id if company_id is not None else account.company_id
            account.company_secret = company_secret if company_secret is not None else account.company_secret
            account.save()

            # 계정이 없다면 새로 만든다.
        except Account.DoesNotExist:
            account = Account.objects.create(user_id = user_id, company_id=company_id, company_secret=company_secret, company= company)
            account.save()
        ...
        if dic:
            ...
            try:
                ...
                for code, value in dic.items():
                    stock = Stock.objects.create(user_id=user_id, code=code, amount=value[1], stockname=value[0], company=1)
                    stock.save()
                    result = True
...
class Query(graphene.ObjectType):
    ...
    def resolve_totalamount(self, info, **kwargs):
        ...
        totalamount = Stock.objects.filter(code=code).aggregate(Sum('amount'))['amount__sum']
        return totalamount
