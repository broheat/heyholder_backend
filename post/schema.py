...
class CreatePostMutation(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        post = Post.objects.create(user=user, code=code, title=title, contents=contents, amount= amount)
        post.save()
...
class Create2CommentMutaion(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        user = info.context.user
        try:
            comment = Comment.objects.create(user=user, post=id, contents=contents, parentComment=parentComment)
            comment.save()
        except Post.DoesNotExist:
